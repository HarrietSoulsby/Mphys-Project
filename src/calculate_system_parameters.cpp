...
OutputParameters calculate_system_parameters(const SystemParameters system_params, const double angle, const double satellite_distance)
{
	...
	double SKR = calculate_skr(transmissivity, PLOB_bound, system_params);
	...
	return {10.0 * std::log10(transmissivity), PLOB_bound, SKR, scintillation, diffraction.transmissivity, diffraction.beam_widening, diffraction.beam_wander, std::pow(1.46 * system_params.wavenumber_laser * system_params.wavenumber_laser * satellite_distance * turbulence, -3.0/5.0), extinction};
}
